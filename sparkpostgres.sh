...
export SPARK_DRIVER_NAME=postgresTospark
...
kubectl exec -it deployment.apps/spark-master -n $SPARK_NAMESPACE -- /bin/bash -c "/opt/spark/bin/spark-submit --name postgresToSparkPipeLine \
   --master $K8ip:$port \
  --deploy-mode cluster  \
  ...
  local:///opt/spark/examples/jars/GCP-1.0-SNAPSHOT-jar-with-dependencies.jar $KafkaBootstrapIP $cassandraHost $cassandraUserName $cassandraPassword $mySQLIP $MySqlUserName $MySqlPassword $KafkaSourceIP"
